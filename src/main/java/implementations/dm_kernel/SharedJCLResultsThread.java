...
public class SharedJCLResultsThread extends Thread{
	
	protected final int port;	
	protected final Selector selector;
	protected final ServerSocketChannel serverSocket;
	/** 3.0 begin **/
	private Map<Long, JCL_result> localMemory;
	/** 3.0 end **/
	
	public SharedJCLResultsThread(int port, Map<Long, JCL_result> localMemory) throws IOException{
		this.port = 4952;
	...
	public void run(){
		...
		Consumer<Long, byte[]> kafkaConsumer = new KafkaConsumer<>(consumerProperties);
		...
		ConsumerRecords<Long, byte[]> consumedRecords = kafkaConsumer.poll(Duration.ofNanos(1));
        		...
        		consumedRecords.forEach(
    				(record) -> {
    					this.localMemory.put(
    							record.key(), 
    							(JCL_result) serializer.deserialize(record.value()));
